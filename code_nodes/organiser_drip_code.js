// Code node for BLKOUT NXT QTIPOC Organiser Drip Campaign workflow
...
// Filter for QTIPOC Organiser members in the drip campaign who are due for next email
const eligibleMembers = members.filter(member => {
  ...
  // Check if they're a QTIPOC Organiser
  const role = (getMemberProp(member, 'Role', '')).toString().toLowerCase();
  const isOrganiser = role.includes('qtipoc organiser');
  ...
  if (isOrganiser && hasCompletedSurvey && !inDripCampaign) {
    return true;
  }
  
  // If they're in the drip campaign and due for next email, include them
  return isOrganiser && inDripCampaign && isDue && dripStage >= 0 && dripStage < 5; // We have 5 emails in the drip campaign
});

console.log(`Found ${eligibleMembers.length} QTIPOC Organiser members due for drip emails`);
