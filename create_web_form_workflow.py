...
def create_web_form_workflow():
    """Create a web form integration workflow using the n8n API."""
    
    # Define the workflow
    workflow = {
        "name": "BLKOUT NXT Web Form Integration (Final)",
        "nodes": [
            {
                "parameters": {
                    "httpMethod": "POST",
                    "path": "blkout-nxt-signup-final",
                    "options": {
...
return memberData;"""
    ...
    try:
        ...
        print(f"Webhook URL: http://localhost:5678/webhook/blkout-nxt-signup-final")
...
if __name__ == "__main__":
    workflow_id = create_web_form_workflow()
    
    if workflow_id:
        print("\nNow you can test the web form integration by sending a POST request to the webhook URL.")
        print("You can use the test_webhook.py script to test the web form integration.")
        print("Example:")
        print("python test_webhook.py --webhook-url http://localhost:5678/webhook/blkout-nxt-signup-final")
