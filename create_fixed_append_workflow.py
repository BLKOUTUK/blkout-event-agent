...
def create_fixed_append_workflow():
    """Create a workflow that correctly appends data to the Google Sheet."""

    # Define the workflow
    workflow = {
        "name": "Fixed Google Sheet Append Test",
        "nodes": [
            {
                "parameters": {
                    "rule": {
            ...
            {
                "parameters": {
                    "jsCode": """// Generate test data with timestamp to make it unique
const timestamp = Date.now();

// Create the test data
return {
  Email: `fixed.append.${timestamp}@example.com`,
  FirstName: "Fixed",
  LastName: "Append",
  Role: "Test",
  ...
  EmailHistory: "[]",
  OptOut: false,
  Source: `Fixed Append Test ${new Date().toISOString()}`
};"""
                },
            ...
            },
            {
                "parameters": {
                    ...
                    "operation": "append", # Changed from appendOrUpdate to append
...
if __name__ == "__main__":
    workflow_id = create_fixed_append_workflow()
